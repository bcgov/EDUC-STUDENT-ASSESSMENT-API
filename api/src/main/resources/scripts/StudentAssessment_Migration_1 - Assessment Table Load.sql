...
SELECT
    ...
    TRIM(assmnt_stud.LOCAL_ASSMT_ID) as LOCAL_ASSMT_ID,
    TRIM(assmnt_stud.MINCODE_HOME) as MINCODE_HOME,
    TRIM(assmnt_stud.MINCODE_ASSMT) as MINCODE_ASSMT,
    TRIM(assmnt_stud.SPECIAL_CASE) as SPECIAL_CASE,
    TRIM(assmnt_stud.WROTE_FLAG) as WROTE_FLAG,
    TRIM(assmnt_stud.GRAD_REQT_MET) as GRAD_REQT_MET,
    TRIM(assmnt_stud.EXCEEDED_WRITES_FLAG) as EXCEEDED_WRITES_FLAG,
    TRIM(assmnt_stud.PERCENT_COMPLETE) as PERCENT_COMPLETE,
    TRIM(assmnt_stud.ASSMT_PROFICIENCY_SCORE) as ASSMT_PROFICIENCY_SCORE,
    TRIM(assmnt_stud.RAW_SCORE) as RAW_SCORE,
    TRIM(assmnt_stud.IRT_SCORE) as IRT_SCORE,
    TRIM(assmnt_stud.ASSMT_PRINT_CODE) as ASSMT_PRINT_CODE,
    TRIM(assmnt_stud.PREPRINT_FLAG) as PREPRINT_FLAG,
    TRIM(assmnt_stud.MC_TOTAL) as MC_TOTAL,
    TRIM(assmnt_stud.OE_TOTAL) as OE_TOTAL,
    TRIM(assmnt_stud.ADAPTED_ASSMT) as ADAPTED_ASSMT,
...
SELECT
    ...
    TRIM(assmnt_comp.CSID) as CSID,
    TRIM(assmnt_comp.SEQN) as SEQN,
    TRIM(assmnt_comp.MC_OE_FLAG) as COMPONENT_TYPE_CODE,
    TRIM(assmnt_comp.COMPONENT_TYPE) as COMPONENT_SUB_TYPE_CODE,
    TRIM(assmnt_comp.BATCH) as BATCH,
...
CREATE INDEX ASSESSMENT_STUDENT_COMPONENT_INDX
    ON ASSESSMENT_STUDENT_COMPONENT(ASSESSMENT_STUDENT_ID);
...
CREATE INDEX ASSESSMENT_STUDENT_ID_INDX
    ON ASSESSMENT_STUDENT(ASSESSMENT_STUDENT_ID);
...
CREATE TABLE ASSESSMENT_STUDENT_RESPONSE
as
SELECT
    LOWER(REGEXP_REPLACE(dbms_crypto.randombytes(16), '(.{8})(.{4})(.{4})(.{4})(.{12})', '\1-\2-\3-\4-\5')) as ASSESSMENT_STUDENT_RESPONSE_ID,
    (SELECT stud.ASSESSMENT_STUDENT_ID FROM ASSESSMENT_SESSION sess, ASSESSMENT assmnt, ASSESSMENT_STUDENT stud, ASSESSMENT_STUDENT_COMPONENT comp
     WHERE sess.COURSE_YEAR = SUBSTR( TRIM(assmnt_mc.ASSMT_SESSION), 0 , 4)
       ...
       AND assmnt.SESSION_ID = sess.SESSION_ID
       AND TRIM(assmnt_mc.COMPONENT_TYPE) = comp.COMPONENT_SUB_TYPE_CODE
       AND TRIM(assmnt_mc.MC_OE_FLAG) = comp.COMPONENT_TYPE_CODE
       AND stud.ASSESSMENT_ID = assmnt.ASSESSMENT_ID
       AND stud.ASSESSMENT_STUDENT_ID = comp.ASSESSMENT_STUDENT_ID
       AND TRIM(assmnt_mc.STUD_NO) = stud.PEN
       AND TRIM(assmnt_mc.ASSMT_CODE) = assmnt.ASSESSMENT_TYPE_CODE) AS ASSESSMENT_STUDENT_COMPONENT_ID,
    TRIM(assmnt_mc.ITEM_NUMBER) as ITEM_NUMBER,
    TRIM(assmnt_mc.QUES_NUMBER) as QUES_NUMBER,
    TRIM(assmnt_mc.MC_ASSMNT_RESP_UNSORTED) as MC_ASSMNT_RESP_UNSORTED,
    TRIM(assmnt_mc.MC_ASSMNT_RESP_SORTED) as MC_ASSMNT_RESP_SORTED,
