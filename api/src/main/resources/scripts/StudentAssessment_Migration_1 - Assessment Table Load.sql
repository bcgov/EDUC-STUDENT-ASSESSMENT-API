...
SELECT
    ...
    (SELECT schl.SCHOOL_ID from SCHOOL schl WHERE SUBSTR( TRIM(assmnt_stud.MINCODE_HOME), 4) = schl.SCHOOL_NUMBER AND SUBSTR( TRIM(assmnt_stud.MINCODE_HOME), 0 , 3) = (SELECT dist.DISTRICT_NUMBER from DISTRICT dist WHERE schl.DISTRICT_ID = dist.DISTRICT_ID)) as SCHOOL_ID,
    (SELECT schl.SCHOOL_ID from SCHOOL schl WHERE SUBSTR( TRIM(assmnt_stud.MINCODE_ASSMT), 4) = schl.SCHOOL_NUMBER AND SUBSTR( TRIM(assmnt_stud.MINCODE_ASSMT), 0 , 3) = (SELECT dist.DISTRICT_NUMBER from DISTRICT dist WHERE schl.DISTRICT_ID = dist.DISTRICT_ID)) as ASSESSMENT_CENTER_ID,
