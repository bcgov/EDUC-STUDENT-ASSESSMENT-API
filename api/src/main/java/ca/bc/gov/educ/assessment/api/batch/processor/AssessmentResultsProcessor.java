...
public class AssessmentResultsProcessor {
    ...
    public AssessmentResultsProcessor(FileValidator fileValidator) {
    ...
    public void processAssessmentResults(AssessmentResultFileUpload fileUpload, String session) {
        ...
        try {
            ...
            var validationError = ValidationUtil.createFieldError(ASSESSMENT_RESULT_UPLOAD, session, fileUnProcessableException.getReason());
            ...
            ApiError error = ApiError.builder().timestamp(LocalDateTime.now()).message(INVALID_PAYLOAD_MSG).status(BAD_REQUEST).build();
            var validationError = ValidationUtil.createFieldError(ASSESSMENT_RESULT_UPLOAD, session , FileError.GENERIC_ERROR_MESSAGE.getMessage());
            List<FieldError> fieldErrorList = new ArrayList<>();
            fieldErrorList.add(validationError);
            error.addValidationErrors(fieldErrorList);
            throw new InvalidPayloadException(error);
    ...
    private void populateBatchFileAndLoadData(String guid, DataSet ds, AssessmentResultFileUpload fileUpload) throws FileUnProcessableException {
        val batchFile = new AssessmentKeyFile();
        populateAssessmentKeyFile(guid, ds, batchFile);
        //load
    }

    public void populateAssessmentResultFile(final String guid, final DataSet ds, final AssessmentResultFile batchFile) throws FileUnProcessableException {
        long index = 0;
        while (ds.next()) {
    ...
    public AssessmentQuestionEntity processLoadedRecordsInBatchFile(@NonNull final String guid, @NonNull final AssessmentResultFile batchFile, @NonNull final AssessmentResultFileUpload fileUpload, final String schoolID, final String districtID) throws FileUnProcessableException {
