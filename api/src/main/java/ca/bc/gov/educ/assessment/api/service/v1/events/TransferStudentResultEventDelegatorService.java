...
@Service
@Slf4j
public class TransferStudentResultEventDelegatorService implements EventHandler {
  /**
   ...
   */
  @Autowired
  public TransferStudentResultEventDelegatorService(final EventHandlerService eventHandlerService, final EventPublisherService eventPublisherService) {
  ...
  public void handleEvent(final Event event) {
    try {
      if(event.getEventType() == EventType.TRANSFER_STUDENT_RESULT) {
        log.debug("Received read from topic event :: ");
        log.trace(PAYLOAD_LOG, event.getEventPayload());
        this.getEventHandlerService().handleProcessTransferStudentResultEvent(event); // no response in this event.
      }
      else {
  ...
  public String getTopicToSubscribe() {
    return TopicsEnum.READ_TRANSFER_STUDENT_TOPIC.toString();
  }
