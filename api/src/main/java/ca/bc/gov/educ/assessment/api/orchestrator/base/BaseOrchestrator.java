...
import ca.bc.gov.educ.assessment.api.struct.NotificationEvent;
...
import static ca.bc.gov.educ.assessment.api.constants.EventOutcome.INITIATE_SUCCESS;
...
import static ca.bc.gov.educ.assessment.api.constants.SagaStatusEnum.COMPLETED;
...
public abstract class BaseOrchestrator<T> implements EventHandler, Orchestrator {
  ...
  public BaseOrchestrator<T> end(final EventType currentEvent, final EventOutcome outcome, final SagaStep<T> stepToExecute) {
    return this.registerStepToExecute(currentEvent, outcome, (T sagaData) -> true, MARK_SAGA_COMPLETE, (Event event, AssessmentSagaEntity saga, T sagaData) -> {
   ...
   */
  protected boolean isNotProcessedEvent(final EventType currentEventType, final AssessmentSagaEntity saga, final Set<EventType> eventTypes) {
   ...
   * @return {@link AssessmentSagaEventStatesEntity}
   */
  protected AssessmentSagaEventStatesEntity createEventState(@NotNull final AssessmentSagaEntity saga, @NotNull final EventType eventType, @NotNull final EventOutcome eventOutcome, final String eventPayload) {
    final var user = this.sagaName.length() > 32 ? this.sagaName.substring(0, 32) : this.sagaName;
    return AssessmentSagaEventStatesEntity.builder()
   ...
   */
  protected void markSagaComplete(final Event event, final AssessmentSagaEntity saga, final T sagaData) {
    this.markSagaComplete(event, saga, sagaData, "");
  }
   ...
   */
  protected void markSagaComplete(final Event event, final AssessmentSagaEntity saga, final T sagaData, final String payloadToSubscribers) {
    ...
    final AssessmentSagaEventStatesEntity sagaEventStates = this.createEventState(saga, event.getEventType(), event.getEventOutcome(), event.getEventPayload());
   ...
   */
  private int calculateStep(final AssessmentSagaEntity saga) {
   ...
   * @return {@link AssessmentSagaEventStatesEntity} if found else null.
   */
  protected Optional<AssessmentSagaEventStatesEntity> findTheLastEventOccurred(final List<AssessmentSagaEventStatesEntity> eventStates) {
    final int step = eventStates.stream().map(AssessmentSagaEventStatesEntity::getSagaStepNumber).mapToInt(x -> x).max().orElse(0);
   ...
   * @throws IOException          if there is connectivity problem
   */
  private void replayFromBeginning(final AssessmentSagaEntity saga, final T t) throws InterruptedException, TimeoutException, IOException {
    final Event event = Event.builder()
  ...
  public void startSaga(@NotNull final AssessmentSagaEntity saga) {
    try {
  ...
  @Override
  @Transactional
  public AssessmentSagaEntity createSaga(@NotNull final String payload, final String userName, final UUID assessmentStudentID, final UUID stagedStudentResultId, final UUID assessmentID, final String pen) {
    return this.sagaService.createSagaRecordInDB(this.sagaName, userName, payload, assessmentStudentID, stagedStudentResultId, assessmentID, pen);
  }

  @Transactional
  public List<AssessmentSagaEntity> createSagas(final List<AssessmentSagaEntity> sagaEntities) {
   ...
   * @throws IOException          if there is connectivity problem
   */
  protected void process(@NotNull final Event event, final AssessmentSagaEntity saga, final T sagaData, final SagaEventState<T> sagaEventState) throws InterruptedException, TimeoutException, IOException {
   ...
   * @throws IOException          if there is connectivity problem
   */
  protected void invokeNextEvent(final Event event, final AssessmentSagaEntity saga, final T sagaData, final SagaEventState<T> sagaEventState) throws InterruptedException, TimeoutException, IOException {
