...
import ca.bc.gov.educ.eas.api.struct.NotificationEvent;
...
import static ca.bc.gov.educ.eas.api.constants.EventOutcome.INITIATE_SUCCESS;
import static ca.bc.gov.educ.eas.api.constants.EventOutcome.SAGA_COMPLETED;
import static ca.bc.gov.educ.eas.api.constants.EventType.INITIATED;
import static ca.bc.gov.educ.eas.api.constants.EventType.MARK_SAGA_COMPLETE;
import static ca.bc.gov.educ.eas.api.constants.SagaStatusEnum.COMPLETED;
...
public abstract class BaseOrchestrator<T> implements EventHandler, Orchestrator {
  ...
  public BaseOrchestrator<T> end(final EventType currentEvent, final EventOutcome outcome, final SagaStep<T> stepToExecute) {
    return this.registerStepToExecute(currentEvent, outcome, (T sagaData) -> true, MARK_SAGA_COMPLETE, (Event event, EasSagaEntity saga, T sagaData) -> {
   ...
   */
  protected boolean isNotProcessedEvent(final EventType currentEventType, final EasSagaEntity saga, final Set<EventType> eventTypes) {
   ...
   */
  protected SagaEventStatesEntity createEventState(@NotNull final EasSagaEntity saga, @NotNull final EventType eventType, @NotNull final EventOutcome eventOutcome, final String eventPayload) {
   ...
   */
  protected void markSagaComplete(final Event event, final EasSagaEntity saga, final T sagaData) {
    this.markSagaComplete(event, saga, sagaData, "");
  }
   ...
   */
  protected void markSagaComplete(final Event event, final EasSagaEntity saga, final T sagaData, final String payloadToSubscribers) {
   ...
   */
  private int calculateStep(final EasSagaEntity saga) {
  ...
  public void replaySaga(final EasSagaEntity saga) throws IOException, InterruptedException, TimeoutException {
   ...
   * @throws IOException          if there is connectivity problem
   */
  private void replayFromLastEvent(final EasSagaEntity saga, final List<SagaEventStatesEntity> eventStates, final T t) throws InterruptedException, TimeoutException, IOException {
   ...
   */
  private void findAndInvokeNextStep(final EasSagaEntity saga, final T t, final EventType currentEvent, final EventOutcome eventOutcome, final Event event) throws InterruptedException, TimeoutException, IOException {
   ...
   * @throws IOException          if there is connectivity problem
   */
  private void replayFromBeginning(final EasSagaEntity saga, final T t) throws InterruptedException, TimeoutException, IOException {
    final Event event = Event.builder()
  ...
  public void startSaga(@NotNull final EasSagaEntity saga) {
    try {
  ...
  @Override
  @Transactional
  public EasSagaEntity createSaga(@NotNull final String payload, final String userName, final UUID assessmentStudentID) {
    return this.sagaService.createSagaRecordInDB(this.sagaName, userName, payload, assessmentStudentID);
  }

  @Transactional
  public List<EasSagaEntity> createSagas(final List<EasSagaEntity> sagaEntities) {
   ...
   * @throws IOException          if there is connectivity problem
   */
  protected void process(@NotNull final Event event, final EasSagaEntity saga, final T sagaData, final SagaEventState<T> sagaEventState) throws InterruptedException, TimeoutException, IOException {
   ...
   * @throws IOException          if there is connectivity problem
   */
  protected void invokeNextEvent(final Event event, final EasSagaEntity saga, final T sagaData, final SagaEventState<T> sagaEventState) throws InterruptedException, TimeoutException, IOException {
