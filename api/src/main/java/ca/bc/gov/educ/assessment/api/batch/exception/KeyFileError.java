...
public enum KeyFileError {
  ...
  QUES_NUM_NOT_NUMERIC("Question number must be numeric on line $?."),
  ...
  MARK_NOT_NUMERIC("Mark value must be numeric on line $?."),
  ...
  TASK_CODE_NOT_ALLOWED("Task code must be blank, A, E, or I on line $?."),
  ...
  SCALE_FACTOR_NOT_NUMERIC("Scale factor must be numeric on line $?."),
  SCALE_FACTOR_NOT_IN_RANGE("Scale factor must be between -99.99 and 99.99 on line $?."),
  ASSMT_SECTION_LENGTH_ERROR("Assessment section must not be longer than 8 on line $?."),
  TOPIC_TYPE_LENGTH_ERROR("Topic type must not be longer than 1 on line $?."),
  ITEM_LENGTH_ERROR("Item must not be longer than 4 on line $?."),
  IRT_COLUMN_LENGTH_ERROR("IRT column must not be longer than 3 on line $?."),
  QUES_ORIGIN_LENGTH_ERROR("Question origin must not be longer than 160 on line $?."),
