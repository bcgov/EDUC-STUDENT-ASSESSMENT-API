...
class ReportsControllerTest extends BaseAssessmentAPITest {
    ...
    @Test
    void testGetDownloadableReport_DetailedDOARByDistrict_WhenNoResults_ShouldReturnPreconditionRequired() throws Exception {
        final GrantedAuthority grantedAuthority = () -> "SCOPE_READ_ASSESSMENT_REPORT";
        final OidcLoginRequestPostProcessor mockAuthority = oidcLogin().authorities(grantedAuthority);
        ...
        session.setCourseMonth("08");
        AssessmentSessionEntity sessionEntity = assessmentSessionRepository.save(session);
        assessmentRepository.save(createMockAssessmentEntity(sessionEntity, "NME10"));

        this.mockMvc.perform(
                        get(URL.BASE_URL_REPORT + "/" + sessionEntity.getSessionID() + "/district/" + district.getDistrictId() + "/nme-detailed-doar/download")
                                .with(mockAuthority))
                .andDo(print()).andExpect(status().isPreconditionRequired());
    }

    @Test
    void testGetDownloadableReport_DOARSummaryByDistrict_ShouldReturnPDFFile() throws Exception {
        final GrantedAuthority grantedAuthority = () -> "SCOPE_READ_ASSESSMENT_REPORT";
        final OidcLoginRequestPostProcessor mockAuthority = oidcLogin().authorities(grantedAuthority);
        ...
        for (var school : List.of(school1, school2)) {
            ...
            studentEntity.setProficiencyScore(1);
    ...
    @Test
    void testGetDownloadableReport_DOARSummaryByDistrict_WhenNoResults_ShouldReturnPreconditionRequired() throws Exception {
        final GrantedAuthority grantedAuthority = () -> "SCOPE_READ_ASSESSMENT_REPORT";
        final OidcLoginRequestPostProcessor mockAuthority = oidcLogin().authorities(grantedAuthority);
    ...
    @Test
    void testCheckDistrictReportAvailability_WithAssessmentTypeCode_WhenResultsExist_ReturnsTrue() throws Exception {
        final GrantedAuthority grantedAuthority = () -> "SCOPE_READ_ASSESSMENT_REPORT";
        final OidcLoginRequestPostProcessor mockAuthority = oidcLogin().authorities(grantedAuthority);
        ...
        this.mockMvc.perform(
                        get(URL.BASE_URL_REPORT + "/" + savedSession.getSessionID() + "/district/" + district.getDistrictId() + "/results/available?assessmentTypeCode=NME10")
                                .with(mockAuthority))
                .andDo(print())
                .andExpect(status().isOk())
                .andExpect(content().string("true"));
    ...
    @Test
    void testCheckDistrictReportTypeAvailability_DoarSummary_WhenResultsExist_ReturnsTrue() throws Exception {
        final GrantedAuthority grantedAuthority = () -> "SCOPE_READ_ASSESSMENT_REPORT";
        final OidcLoginRequestPostProcessor mockAuthority = oidcLogin().authorities(grantedAuthority);
